...
/// is not at the end of the list. [groupSize] is how many gestures it holds: a
/// group taller than the viewport keeps its header pinned while the list is
/// scrolled inside it.
Config _listConfig({
  List<String> names = const ['First', 'Second', 'Third'],
  int? groupAt,
  int groupSize = 2,
}) {
  final nodes = [
      ...
      GestureGroupNode(
        ...
        children: [
          for (var i = 0; i < groupSize; i++)
            GestureNode.leaf(PressGesture(common: TriggerCommon(name: 'H$i'))),
        ],
      ),
    );
  }
  return Config(mouseNodes: nodes);
}
