...
Widget _host(List<TriggerAction> actions) => themedApp(
  FScaffold(
...
void main() {
  group('flow', () {
    ...
    testWidgets('unfolds sub-groups under a dotted step number', (
      tester,
    ) async {
      tester.view
  ...
  group('reveal', () {
    final nested = TriggerCommon(
      actions: [
        TriggerAction(
          action: ActionGroup(actions: [cmd('child one'), cmd('child two')]),
        ),
      ],
    );
    ...
    testWidgets('a step number opens the card of the row it stands for', (
      ...
      await pumpAnchoredActionList(tester, nested, size: const Size(1000, 900));
      ...
      expect(rowFooters, findsNWidgets(2));
      expect(find.text('child two'), findsNWidgets(2));

      // Let go once settled, so reopening by hand does not flash again.
